...
class ProposalRepoSql(protected[sql] val xa: doobie.Transactor[IO]) extends GenericRepo with ProposalRepo {
  ...
  override def countVotes(id: Proposal.Id): IO[Seq[Proposal.Vote.Rating]] = selectVotes(id).runList(xa).map(_.flatten)
...
object ProposalRepoSql {
  ...
  private val voteTable = Tables.voteProposals
  //  private val voteTableWithUser = voteTable
  ...
  private[sql] def insertVote(e: Proposal.Vote): Insert[Proposal.Vote] =
    voteTable.insert[Proposal.Vote](e, _ => fr0"${e.proposal}, ${e.user}, ${e.rating.value}, ${e.voted}")
  ...
  private[sql] def update(e: Proposal.Vote): Update =
    voteTable.update(fr"vote=${e.rating.value}, vote_at=${e.voted}", fr0"WHERE vp.proposal_id=${e.proposal} AND vp.user_id=${e.user}")
  ...
  private[sql] def selectVotes(id: Proposal.Id): Select[Seq[Proposal.Vote.Rating]] =
    voteTable.select[Seq[Proposal.Vote.Rating]](Seq(Field("vote", "vp")), fr0"WHERE vp.proposal_id=$id")
